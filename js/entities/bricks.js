...
class Brick {
    ...
    draw() {
        ...
        if(asset){
            ...
            if (this.isBoss) this.drawBossVisual(ctx,x,y);
        ...
        // 🎨 SPARKLES somente no alto
        if(!Game.settings || Game.settings.effectiveGraphics==='HIGH') this.drawSparkles(ctx, x, y);
...
class BrickManager {
    ...
    loadLevel(level) {
        ...
        this.specialEncounter = (level % 10 === 0) ? 'BOSS' : ((level % 5 === 0) ? 'MINI_BOSS' : null);
        ...
        if (Game.levelGenerator) {
            ...
            // Notifica encontros especiais
            if (level % 5 === 0) {
                if (Game.hud) {
                    Game.hud.addNotification(level % 10 === 0 ? '👾 BOSS LEVEL!' : '⚠ MINI-BOSS!', level % 10 === 0 ? '#FF3B57' : '#ffb347', 3);
                }
                if (Game.stats) {
    ...
    onLevelComplete() {
        ...
        const startNext = () => {
            ...
            Game.ball.reset();
            if (Game.paddle) Game.paddle.reset();
            this.levelCompleting = false;
        };
        if (Game.levelLoader) {
            setTimeout(() => Game.levelLoader.load(nextLevel, startNext), 650);
        } else {
    ...
    draw() {
        ...
        if (this.specialEncounter) this.drawEncounterBar();
    }

    drawEncounterBar() {
        const bosses=this.bricks.filter(b=>b.isBoss && !b.destroyed);
        if(!bosses.length)return;
        const current=bosses.reduce((n,b)=>n+Math.max(0,b.currentHits),0);
        const max=this.bricks.filter(b=>b.isBoss).reduce((n,b)=>n+(b.stats?.hits||1),0)||1;
        const ctx=Game.ctx, x=180,y=53,w=440,h=14,p=Math.max(0,Math.min(1,current/max));
        ctx.save();ctx.fillStyle='rgba(0,0,0,.72)';ctx.fillRect(x-2,y-2,w+4,h+4);
        const g=ctx.createLinearGradient(x,0,x+w,0);g.addColorStop(0,this.specialEncounter==='BOSS'?'#ff3158':'#ff9f43');g.addColorStop(1,'#ffd166');ctx.fillStyle=g;ctx.fillRect(x,y,w*p,h);
        ctx.strokeStyle='rgba(255,255,255,.35)';ctx.strokeRect(x,y,w,h);ctx.textAlign='center';ctx.font='700 11px Orbitron,Arial';ctx.fillStyle='#fff';ctx.fillText(this.specialEncounter==='BOSS'?'BOSS CORE':'MINI-BOSS',Game.width/2,y+11);ctx.restore();
    }
