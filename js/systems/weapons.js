...
class WeaponSystem {
  ...
  fire(){if(!this.enabled||this.cooldown>0||!Game.paddle)return;this.cooldown=.22;const y=Game.paddle.y-8;this.lasers.push({x:Game.paddle.x+12,y,w:4,h:14},{x:Game.paddle.x+Game.paddle.width-16,y,w:4,h:14});if(Game.audio)Game.audio.play('paddleHit');}
  ...
  draw(){const ctx=Game.ctx;ctx.save();for(const l of this.lasers){ctx.fillStyle='#ff4fd8';if(Game.settings?.effectiveGraphics!=='LOW'){ctx.shadowBlur=12;ctx.shadowColor='#ff4fd8';}ctx.fillRect(l.x,l.y,l.w,l.h);}ctx.restore();}
