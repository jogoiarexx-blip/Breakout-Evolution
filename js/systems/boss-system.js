class BossSystem {
  ...
  draw(){
    ...
    for(const p of this.projectiles){const fx=Game.assets?.image('vfx-pulse');ctx.shadowBlur=Game.settings?.effectiveGraphics==='LOW'?0:14;ctx.shadowColor=p.color;if(fx){const z=p.r*5.5;Game.assets.drawContain(ctx,fx,p.x,p.y,z,z,.98);}else{ctx.fillStyle=p.color;ctx.beginPath();ctx.arc(p.x,p.y,p.r,0,Math.PI*2);ctx.fill();}}
    ctx.shadowBlur=0;
  ...
  drawWorldCore(ctx){
    ...
    if(bossSprite){ctx.save();ctx.translate(c.x,c.y);const pulse=1+Math.sin(t*3)*.035;ctx.scale(pulse,pulse);const size=this.kind==='BOSS'?112:86;if(q!=='LOW'){ctx.shadowBlur=24;ctx.shadowColor=['#00d2ff','#ff6238','#8ee7ff','#b388ff','#ffd166'][wi]||'#fff';}Game.assets.drawContain(ctx,bossSprite,0,0,size,size,.98);ctx.restore();return;}
